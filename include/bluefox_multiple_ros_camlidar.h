#ifndef _BLUEFOX_MULTIPLE_ROS_CAMLIDAR_H_
#define _BLUEFOX_MULTIPLE_ROS_CAMLIDAR_H_
...
public:
    ...
    ~BlueFOX_MULTIPLE_ROS_CAMLIDAR();

    void callback(const camlidar_module::trg_msgConstPtr& msg);
...
/* IMPLEMENTATION */
BlueFOX_MULTIPLE_ROS_CAMLIDAR::~BlueFOX_MULTIPLE_ROS_CAMLIDAR(){
    for(int i = 0; i < n_devs_; i++){
        delete bluefoxs_[i];
    }
};
...
void BlueFOX_MULTIPLE_ROS_CAMLIDAR::callbackDynReconfig(bluefox::bluefoxDynConfig &config, uint32_t lvl) {
    if(config.hdr){
        for(int i = 0; i < n_devs_; i++)
